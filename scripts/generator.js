...
function formData() {
    return {
        age: form.querySelector('#age').valueAsNumber,
        name: form.querySelector('#name').value,
        gender: form.querySelector('#genderf').checked ? 'female' : 'male',
        eyeColor: form.querySelector('#eye-color').value,
        hairColor: form.querySelector('#hair-color').value,
        hairLength: form.querySelector('#hair-length').value,
...
form
    .querySelectorAll('input:not([type=button],[type=submit],[type=reset],[type=radio]),select')
    .forEach(e => {
    if (localStorage[`input#${e.id}`])
        e.value = localStorage[`input#${e.id}`];
...
form.querySelectorAll('input[type=radio]').forEach(e => {
    if (localStorage[`input#${e.id}`])
        e.checked = localStorage[`input#${e.id}`];
